...
export default function UpgradeClient({ userEmail }: UpgradeClientProps) {
  const [billing, setBilling] = useState<BillingCycle>("monthly");

  const handleUpgrade = (plan: Plan) => {
    const link = plan.paymentLink[billing];
    if (!link) return; // Free plan or no link available
    window.location.href = `${link}?prefilled_email=${userEmail}`;
  };
      ...
      <section className="relative z-10 mx-auto grid max-w-6xl grid-cols-1 gap-6 px-6 pb-20 sm:px-10 md:grid-cols-3">
        {plans.map((plan) => {
          ...
          return (
            <div
              key={plan.id}
              className={`relative flex flex-col rounded-[20px] border p-8 ${
                plan.featured
            ...
            >
              ...
              <button
                ...
                disabled={isDisabled}
                className={`mb-7 w-full rounded-xl py-3 text-[14.5px] font-bold transition-transform active:scale-[0.98] ${
                  plan.featured
